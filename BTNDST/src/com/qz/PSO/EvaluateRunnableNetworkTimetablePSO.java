...
public class EvaluateRunnableNetworkTimetablePSO implements Runnable {
    ...
    public EvaluateRunnableNetworkTimetablePSO(int ThreadID, int StartIndex, List<Chromosome> SolutionList, List<Double> TmSolutionSufficiency,
        ...
        try {
            ...
            for (int i = 0; i < solutionList.size(); i++) {//遍历所有的解
                ...
                for (int t = 0; t < busLines.size(); t++) {
                    ...
                    if (PSO.solutionPartSufficiency.get(startIndex+i).get(t) == 0) {
                        if (tmFreq > 0) {
                        ...
                        if (tmFreq > 0) {
                            tmSolutionPartSufficiencyPt.add(PSO.b * (PSO.solutionPartSufficiency.get(startIndex+i).get(t)) + (1 - PSO.b) * tmSufficiency);//累积片段适应度
                        } else {//此线路没有提供贡献，要给惩罚
                            tmSolutionPartSufficiencyPt.add(1.2 * (PSO.b * (PSO.solutionPartSufficiency.get(startIndex+i).get(t)) + (1 - PSO.b) * tmSufficiency));//累积片段适应度
                        }
                    }
                }
                ...
                if (PSO.BestCost < tmSufficiency) {
                    PSO.BestCost = tmSufficiency;
                    PSO.BestBusLine = busLines;
                    PSO.BestSolution = solutionList.get(i);
                    ...
                    PSO.PassengerNotAsignRes = unServedPassengers;
                }
